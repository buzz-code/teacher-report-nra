    ...
    return (
        <FunctionField
            ...
            render={(record) => {
                ...
                return (
                    <span>
                        {component.subtotal.toFixed(2)} {currency}
                    </span>
                );
            }}
        />
    ...
    return (
        <FunctionField
            ...
            render={(record) => {
                ...
                explanationLines.push(
                    `${translate('priceExplanation.total', { _: 'סה"כ' })}: ${currency}${price.toFixed(2)}`,
                );
...
const Datagrid = ({ isAdmin, children, ...props }) => {
    ...
    return (
        <CommonDatagrid {...props} additionalBulkButtons={additionalBulkButtons}>
            ...
            {shouldShowField('howManyLessons', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyLessons" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* SEMINAR_KITA, PDS */}
            {shouldShowField('howManyWatchOrIndividual', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyWatchOrIndividual" priceMap={priceMap} isLoading={isLoading} />
            )}
            {shouldShowField('howManyTeachedOrInterfering', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyTeachedOrInterfering" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* SEMINAR_KITA, MANHA, PDS */}
            {shouldShowField('howManyDiscussingLessons', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyDiscussingLessons" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* SEMINAR_KITA only */}
            {shouldShowField('wasKamal', selectedTeacherTypeKey) && (
                <FieldPriceField source="wasKamal" priceMap={priceMap} isLoading={isLoading} />
            )}
            {shouldShowField('howManyLessonsAbsence', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyLessonsAbsence" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* MANHA only */}
            {shouldShowField('howManyMethodic', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyMethodic" priceMap={priceMap} isLoading={isLoading} />
            )}
      ...
      {shouldShowField('howManyStudentsHelpTeached', selectedTeacherTypeKey) && <FieldPriceField source="howManyStudentsHelpTeached" priceMap={priceMap} isLoading={isLoading} />} */}
            ...
            {shouldShowField('howManyStudentsTeached', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyStudentsTeached" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* KINDERGARTEN only */}
            {shouldShowField('wasCollectiveWatch', selectedTeacherTypeKey) && (
                <FieldPriceField source="wasCollectiveWatch" priceMap={priceMap} isLoading={isLoading} />
            )}

            {/* SPECIAL_EDUCATION only */}
            {shouldShowField('howManyStudentsWatched', selectedTeacherTypeKey) && (
                <FieldPriceField source="howManyStudentsWatched" priceMap={priceMap} isLoading={isLoading} />
            )}
            {shouldShowField('wasPhoneDiscussing', selectedTeacherTypeKey) && (
                <FieldPriceField source="wasPhoneDiscussing" priceMap={priceMap} isLoading={isLoading} />
            )}
