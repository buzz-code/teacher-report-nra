...
const TypeChip = ({ source }) => {
  ...
  const typeLabel = translate(`resources.reportable_item_with_price.values.type.${typeKey}`);

  return <Chip label={typeLabel} color={isAnswer ? 'secondary' : 'primary'} size="small" />;
};

const additionalBulkButtons = [
  <BulkActionButton name="assignToSalaryReport" label="הקצה לדוח שכר" icon={<AssignmentIcon />} reloadOnEnd>
...
const Datagrid = ({ isAdmin, children, ...props }) => {
  return (
    <CommonDatagrid {...props} readonly additionalBulkButtons={additionalBulkButtons}>
      ...
      <FunctionField source="type" render={TypeChip} />
      <ReferenceField source="teacherReferenceId" reference="teacher" />
      <DateField source="reportDate" />
      <NumberField source="calculatedPrice" options={currencyOptions} />
